-- STEP 15.5 finding: p_is_production on create_order() (STEP 15) was meant
-- to stop an unverified DEV_EXCHANGE_RATES-converted price from ever being
-- charged in production, and lib/actions/order.ts does set it correctly
-- from the server's own process.env.NODE_ENV — but create_order() is
-- `grant`ed to `anon`, so anyone holding the public anon key can call it
-- directly over PostgREST with p_is_production explicitly set back to
-- false, bypassing the Server Action (and this whole guard) entirely. A
-- parameter the caller supplies can never be the trust boundary for "is
-- this a production database" — this was only caught by actually calling
-- the RPC directly against a real Cloud project (STEP 15.5), the exact
-- kind of check STEP 15's own raw-Postgres testing couldn't perform.
--
-- Fix: the RPC now reads whether it's production from app_settings
-- (SECURITY DEFINER already bypasses that table's admin-only RLS, so this
-- doesn't need a new policy) instead of trusting a parameter — an operator
-- setting, not a request parameter, is the only thing that can decide it.
-- p_is_production is dropped from the signature; the old 13-param
-- signature is dropped first per Postgres's function-overload rules.
--
-- Defaults to true (fail-closed: refuse unverified dev-rate pricing)
-- everywhere this migration runs, including a fresh dev/staging project —
-- flip it explicitly once per environment:
--   update public.app_settings set value = 'false' where key = 'orders.is_production';
insert into public.app_settings (key, value) values ('orders.is_production', 'true')
on conflict (key) do nothing;

drop function if exists public.create_order(
  text, uuid, text, text, public.market_code_enum, public.currency_code_enum,
  public.payment_method_enum, jsonb, jsonb, jsonb, text, integer, boolean
);
